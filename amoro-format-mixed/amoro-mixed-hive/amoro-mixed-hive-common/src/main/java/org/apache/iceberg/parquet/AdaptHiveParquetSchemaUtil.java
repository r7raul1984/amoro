...
public class AdaptHiveParquetSchemaUtil {
  ...
  private static Schema convertInternal(
    ...
    public static MessageType pruneColumns(MessageType fileSchema, Schema expectedSchema) {
        ...
        ServiceLoader<HiveParquetSchemaConvertAdapter> loader = ServiceLoader.load(HiveParquetSchemaConvertAdapter.class);
        HiveParquetSchemaConvertAdapter hiveParquetSchemaConvert = loader.findFirst()
                .orElseThrow(() -> new IllegalStateException("No HiveParquetSchemaConvertAdapter providers found"));
        return hiveParquetSchemaConvert.pruneColumns(fileSchema, expectedSchema);
    }
